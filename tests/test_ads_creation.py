...
def test_create_ad_unauthorized_shows_auth_popup(driver):
#Перейти на стенд
    driver.get(URL)
...
def test_create_ad_authorized_user(driver,random_email, ad_name, ad_description, ad_price, city, category, condition, test_password = 'Test1234'):
#Перейти на стенд
    driver.get(URL)
    WebDriverWait(driver, 5).until(expected_conditions.visibility_of_element_located(LOGIN_AND_REGISTRATION_BUTTON))
